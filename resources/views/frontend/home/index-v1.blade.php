...
@section('content')
    ...
    <section class="banner-area bg_cover lazy"
        data-bg="{{ !empty($heroInfo->background_image) ? asset('assets/img/hero-section/' . $heroInfo->background_image) : asset('assets/img/static/hero.jpeg') }}">
        <div class="container">
            <div class="row">
                <div class="col-lg-7">
                    <div class="banner-content">
                        ...
                        <ul>
                            @if (!empty($heroInfo->first_button) && !empty($heroInfo->first_button_url))
                                <li><a class="main-btn"
                                        href="{{ $heroInfo->first_button_url }}">{{ $heroInfo->first_button }}</a></li>
                            @endif

                            @if (!empty($heroInfo->second_button) && !empty($heroInfo->second_button_url))
                                <li><a class="main-btn-2 main-btn"
                                        href="{{ $heroInfo->second_button_url }}">{{ $heroInfo->second_button }}</a></li>
                            @endif
                        </ul>
                    </div>
                </div>
    ...
    @if ($secInfo->call_to_action_section_status == 1)
        <section class="offer-area bg_cover pt-110 pb-120 lazy"
            @if (!empty($callToActionInfo)) data-bg="{{ asset('assets/img/action-section/' . $callToActionInfo->background_image) }}" @endif>
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-11">
                                ...
                                <div class="single-courses mt-30">
                                    <div class="courses-thumb">
                                        <a href="{{ route('course_details', ['slug' => $course->slug]) }}" class="d-block">
                                            <img data-src="{{ asset('assets/img/courses/thumbnails/' . $course->thumbnail_image) }}"
                                                class="crsImage lazy" alt="image">
                                        </a>

                                        <div class="corses-thumb-title">
                                            <a class="category"
                                                href="{{ route('courses', ['category' => $course->categorySlug]) }}">{{ $course->categoryName }}</a>
                                        </div>
                                    </div>
                                    ...
                                    <div class="courses-content">
                                        ...
                                        <div class="courses-info d-flex justify-content-between">
                                            <div class="item">
                                                <img data-src="{{ asset('assets/img/instructors/' . $course->instructorImage) }}"
                                        ...
                                        <ul class="d-flex justify-content-center">
                                            <li><i class="fal fa-users"></i> {{ $course->enrolmentCount }}
                                                {{ __('Students') }}</li>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="play-thumb">
                            @if (!empty($videoData))
                                <img data-src="{{ asset('assets/img/video-images/' . $videoData->image) }}"
    ...
    @if ($secInfo->fun_facts_section_status == 1)
        <section class="counter-area bg_cove lazy"
            @if (!empty($factData->background_image)) data-bg="{{ asset('assets/img/fact-section/' . $factData->background_image) }}" @endif>
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-7">
                        <div class="section-title text-center">
    ...
    @if ($secInfo->testimonials_section_status == 1)
        <section class="testimonials-area pb-115 bg_cover lazy"
            @if (!empty($testimonialData)) data-bg="{{ asset('assets/img/' . $testimonialData->testimonials_section_image) }}" @endif>
            <div class="container">
                        ...
                        @foreach ($testimonials as $testimonial)
                            <div class="col-lg-12">
                                <div class="testimonials-content text-center">
                                    ...
                                    <img data-src="{{ asset('assets/img/clients/' . $testimonial->image) }}"
        ...
        <section class="community-area">
            ...
            @if (!empty($newsletterData->image))
                <div class="community-thumb d-none d-lg-block">
                    <img data-src="{{ asset('assets/img/newsletter-section/' . $newsletterData->image) }}" class="lazy"
                        alt="image">
                </div>
            @endif
        </section>
