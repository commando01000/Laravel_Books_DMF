...
class UserController extends Controller
{
  ...
  public function downloadFile($id)
  {
    ...
    $pathToFile = public_path('assets/file/lesson-contents/') . $lessonContent->file_unique_name;
  ...
  public function contentCompletion(Request $request)
  {
    ...
    if ($content->type == 'video') {
      $lccCount1 = LessonContentComplete::where('user_id', Auth::guard('web')->id())
        ->where('lesson_id', $content->lesson_id)
        ->where('lesson_content_id', $id)
        ->where('type', 'video')
        ->count();
    ...
    if ($course->video_watching == 1) {
      ...
      if ($totalVideo > 0) {
        $totalCompletedVideo = LessonContentComplete::where('lesson_id', $lessonId)
    ...
    if ($course->quiz_completion == 1) {
      $totalQuiz = $lesson->content()->where('type', 'quiz')->count();
      $quizScore = QuizScore::select('score')->where('course_id', $courseId)
      ...
      if ($content->type == 'quiz' && $quizCompleted == true) {
        $lccCount2 = LessonContentComplete::where('user_id', Auth::guard('web')->id())
          ->where('lesson_id', $content->lesson_id)
          ->where('lesson_content_id', $id)
          ->where('type', 'quiz')
          ->count();
  ...
  public function updateProfile(UserProfileRequest $request)
  {
    ...
    if ($request->hasFile('image')) {
      $imageName = UploadFile::update(public_path('assets/img/users/'), $request->file('image'), $authUser->image);
    }
