...
class EnrolmentController extends Controller
{
  public function enrol(Request $request, $id)
  {
    ...
    if (Auth::guard('web')->check() == false) {
      ...
      if ($user->edit_profile_status == 0) {
        session()->flash('profile_warning', 'Please complete your profile information');
  ...
  public function calculation(Request $request, $courseId)
  {
    ...
    if (session()->has('discountedCourse')) {
      $_course_id = session()->get('discountedCourse');

      if ($courseId == $_course_id) {
        if (session()->has('discount')) {
          $_discount = session()->get('discount');
        }

        if (session()->has('discountedPrice')) {
          $_course_new_price = session()->get('discountedPrice');
        }
      }
    }
  ...
  public function generateInvoice($enrolmentInfo, $courseId)
  {
    ...
    $directory = config('dompdf.public_path');
    ...
    Pdf::loadView('frontend.curriculum.invoice', compact('enrolmentInfo', 'courseInfo'))->save($fileLocated);
  ...
  public function sendMail($enrolmentInfo)
  {
    ...
    try {
      ...
      // Attachments (Invoice)
      $mail->addAttachment(public_path('assets/file/invoices/') . $enrolmentInfo->invoice);
  ...
  public function complete(Request $request, $id, $via = null)
  {
    $language = $this->getLanguage();
    ...
    session()->forget('discountedCourse');
