...
class FunFactController extends Controller
{
  public function index(Request $request)
  {
    ...
    $information['data'] = $language->funFactSec()->first();

    $information['countInfos'] = $language->countInfo()->orderByDesc('id')->get();
  ...
  public function updateSection(Request $request)
  {
    ...
    if (empty($factInfo)) {
      if ($themeInfo->theme_version == 1 || $themeInfo->theme_version == 2) {
        $backgroundImageName = UploadFile::store(public_path('assets/img/fact-section/'), $request->file('background_image'));
      }
      ...
      if ($themeInfo->theme_version == 1 || $themeInfo->theme_version == 2) {
        if ($request->hasFile('background_image')) {
          $backgroundImageName = UploadFile::update(public_path('assets/img/fact-section/'), $request->file('background_image'), $factInfo->background_image);
        }
      }
  ...
  public function bulkDestroy(Request $request)
  {
    ...
    session()->flash('success', 'Informations deleted successfully!');
