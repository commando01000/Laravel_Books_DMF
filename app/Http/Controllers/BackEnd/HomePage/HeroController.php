...
class HeroController extends Controller
{
  ...
  public function update(Request $request)
  {
    ...
    // insert data into db
    if (empty($heroInfo)) {
      $backgroundImageName = UploadFile::store(public_path('assets/img/hero-section/'), $request->file('background_image'));

      $imageName = NULL;

      if ($themeInfo->theme_version == 3 && $request->hasFile('image')) {
        $imageName = UploadFile::store(public_path('assets/img/hero-section/'), $request->file('image'));
      }
      ...
      if ($request->hasFile('background_image')) {
        $backgroundImageName = UploadFile::update(public_path('assets/img/hero-section/'), $request->file('background_image'), $heroInfo->background_image);
      }

      if ($themeInfo->theme_version == 3 && $request->hasFile('image')) {
        $imageName = UploadFile::update(public_path('assets/img/hero-section/'), $request->file('image'), $heroInfo->image);
      }
