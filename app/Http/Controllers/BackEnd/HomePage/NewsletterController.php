...
class NewsletterController extends Controller
{
  public function index(Request $request)
  {
    ...
    $information['data'] = $language->newsletterSec()->first();
  ...
  public function update(Request $request)
  {
    ...
    if (empty($newsletterInfo)) {
      if ($themeInfo->theme_version == 1 || $themeInfo->theme_version == 3) {
        $backgroundImageName = UploadFile::store(public_path('assets/img/newsletter-section/'), $request->file('background_image'));
      }

      if ($themeInfo->theme_version == 1) {
        $imageName = UploadFile::store(public_path('assets/img/newsletter-section/'), $request->file('image'));
      }
      ...
      if ($themeInfo->theme_version == 1 || $themeInfo->theme_version == 3) {
        if ($request->hasFile('background_image')) {
          $backgroundImageName = UploadFile::update(
            public_path('assets/img/newsletter-section/'), 
            $request->file('background_image'), 
            $newsletterInfo->background_image
          );
        }
      }
      ...
      if ($themeInfo->theme_version == 1) {
        if ($request->hasFile('image')) {
          $imageName = UploadFile::update(public_path('assets/img/newsletter-section/'), $request->file('image'), $newsletterInfo->image);
        }
      }
