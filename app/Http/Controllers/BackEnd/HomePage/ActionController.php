...
class ActionController extends Controller
{
  ...
  public function update(Request $request)
  {
    ...
    // store data in db
    if (empty($actionInfo)) {
      $backgroundImageName = UploadFile::store(public_path('assets/img/action-section/'), $request->file('background_image'));

      $imageName = NULL;

      if ($themeInfo->theme_version == 2 && $request->hasFile('image')) {
        $imageName = UploadFile::store(public_path('assets/img/action-section/'), $request->file('image'));
      }
      ...
      if ($request->hasFile('background_image')) {
        $backgroundImageName = UploadFile::update(public_path('assets/img/action-section/'), $request->file('background_image'), $actionInfo->background_image);
      }

      if ($themeInfo->theme_version == 2 && $request->hasFile('image')) {
        $imageName = UploadFile::update(public_path('assets/img/action-section/'), $request->file('image'), $actionInfo->image);
      }
